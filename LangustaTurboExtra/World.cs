    ...
    public class World : GameObject
    {
        ...
        private bool damageMonster(int damage, MapTile mapTile, int x, int y)
        {
            ...
            if (mapTile.ObjectHealth <= 0)
            {
                ...
                //max hp potwora to jego exp
                _gameState.Experience += mapTile.ObjectTile.Health;

                //usun potwora i zamien na grob
                mapTile.ObjectTile = _tiles["bon"];
                mapTile.SetObjectSprite(x, y);
                returnValue = true; //potwor zdechl
            }

            _popups.Add(new textPopup((int)mapTile.Sprite.Location.X + 40, (int)mapTile.Sprite.Location.Y + 20, (damage != 0) ? damage.ToString() : "Pud³o"));
